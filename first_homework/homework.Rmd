...
---
## Немного истории о больших данных и как с ними можно развлекаться
...
## Подгрузка данных и их проверка

Для нашей работы мы загружем файлы "GSE89225_illumina_counts.csv", "conditions.csv", "human_mart.txt" и смотрим на них, осознованя по лекции, что там есть один надоедливый оутлайнер. И смотрим что в них происходит.
...
# Sanity checks
Страхуем себя опять, на случай того, если в библиотеке множесто мРНК, и видим по графику, что все ОК (у кого-то очень прямые руки были).

```{r sanity_check, message = FALSE, warning=FALSE, echo=FALSE}
proteinCoding <- mart[mart[, 3] == "protein_coding", ]
rRNA <- mart[mart[, 3] == "rRNA", ] #rewrite with dplyr maybe
...
И куча всего, с чем нам надо научиться работать. Сложно, интересно.. времени надо дофига.
...
```{r deseq_prep_2, cache=TRUE, message=FALSE, warning=FALSE, echo=FALSE }
...
```{r PCA, warning=FALSE, echo=FALSE, message=FALSE}
...
```{r diff_exp, warning=FALSE, echo=FALSE, message=FALSE}
res <- results(dds)
res
...
```{r volcano_plot, warning=FALSE, echo=FALSE, message=FALSE}
gdata <- data.frame(
...
```{r full heatmap, warning=FALSE, echo=FALSE, message=FALSE}
...
draw.pairwise.venn(length(setA), length(setB), length(commonLetters),
                   category = c("Treg vs Tconv", "Tumor breast vs Normal breast"),  fill = c("light blue", "pink"))
